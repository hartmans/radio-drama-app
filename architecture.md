...
* the recording projection retains inline `DialogueAudio` while the preceding dialogue source is `recording`. It retains `ScriptGap` boundaries when either the preceding or following dialogue line is recorded, including leading gaps and gaps across source transitions
* native and forced-aligned dialogue timing preserve unknown boundaries as NaN, including in the existing TTS cache span fields. Recording and TTS projections share boundary handling; fallback spans are used only for legacy inline-audio placement. A slice reports an unknown boundary when it needs that boundary for placement
* each dialogue line prefers an exact normalized match to whole original transcription segments, then whole acoustically aligned clauses, then word matching. Matches may span consecutive clauses and advance through the recording in order; unrelated unmatched lines do not disable clause matching. Original segment boundaries remain available even when word alignment is required elsewhere
* alignment matching normalizes spoken zero through nine to digit tokens, splits numeric groups into digits, treats spoken point like a decimal point, and ignores hyphens. Authored speech is unchanged; expanded ASR tokens retain their original word timestamps
* alignment requests for projections containing gaps require word timings so resynchronization can match transcript words within the source audio; clause-only shortcuts remain available for projections without gaps. Forced-alignment cache versions invalidate computed timing when the alignment algorithm changes while preserving synthesized audio
...
* `TtsResource` is the backend-independent injectable speech interface; its `register_request()` method accepts a script-level `ScriptRenderRequest` and returns a lazy, cache-mediated registration with asynchronous `render()` and `ensure_timing()` methods
...
* backend output remains in its native format; the cache-mediated registration returns audio in the configured production sample rate and channel layout
...
* `ProxyTtsResource` retains registrations until rendering begins, prepares one normalized reference WAV per distinct resolved source path and gain under `normalized_voices` in the production cache, and launches a persistent Podman container with those prepared references mounted read-only; aliases share a container path. The production cache root is also mounted read-write as `/cache` and used as the container working directory
...
* `radio_drama_tts_container.SpeakerSlots` lets container engines share model speaker slots by reference path and gain, independently of authored speaker names and output effects; reference audio already includes gain applied by the host
...
* proxy responses return cache-relative WAV paths and optional dialogue-line spans; the host rejects absolute and parent-traversing paths and reads the model-native artifact from the production cache
* each configured proxy backend uses the shared script-render cache contract under its own backend name; cache hits are resolved before container startup or capability-driven reference transcription
* `radio_drama_tts_container` is a Python-standard-library-only helper package for container engines; it supplies the protocol loop, deterministic artifact naming, PCM16 WAV writing, and reusable line-oriented WAV assembly, while each `tts_engines/<name>` directory owns its Containerfile and engine-specific dependencies and code

...
* every new TTS engine must honor speaker gain, must consume reference audio processed through the shared voice-preprocessing chain, and must implement efficient batching across the pending script and line work rather than issuing one model request per line serially; proxy engines satisfy the first two requirements by consuming the host-prepared mounted reference WAVs and must provide a genuinely batched container implementation

...
* `tts_engines/higgs_tts_3` adapts proxy dialogue to the SGLang-Omni OpenAI-compatible speech API for `bosonai/higgs-tts-3-4b`; it synthesizes each dialogue line with that line's mounted voice reference, concatenates the model-native 24 kHz WAV segments, and returns parallel line start positions
* the Higgs container declares `/models/huggingface` as its checkpoint volume; its sample proxy configuration bind-mounts a persistent host cache there and explicitly enables the NVIDIA CDI device, host IPC, and the shared-memory allocation used by SGLang-Omni

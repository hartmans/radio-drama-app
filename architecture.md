...
* the Zonos container uses the full v0.1 transformer checkpoint and its native batched generator across pending dialogue lines; it keeps the model and derived speaker embeddings resident, and decodes each batch item only through its own EOS rather than decoding rectangular batch padding

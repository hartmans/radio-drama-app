...
* proxy engines receive all currently pending scripts together and should batch flattened line work through the model when its supported inference API permits it; an upstream batch-size-one decoder may instead serialize inference on one resident model while caching reusable speaker features
...
* the Zonos container uses the full v0.1 transformer checkpoint and its native batched generator across pending dialogue lines; it keeps the model and derived speaker embeddings resident, decodes each batch item only through its own EOS rather than decoding rectangular batch padding, retries stochastic initial-EOS results before an empty token sequence can reach the codec, and uses a removable encoded-silence pre-roll plus a short fade so generation and codec decoding do not begin at an audible discontinuity
* the Chatterbox container uses Multilingual V3 and keeps the model plus per-speaker conditionals resident; its upstream public decoder is batch-size-one, so GPU generation remains serialized while work is flattened across pending scripts
* the VoxCPM2 container keeps one model resident and serializes prompt-conditioned line synthesis; it requests speaker transcripts and uses the prepared reference WAV plus its transcript for high-fidelity prompt cloning, falling back to reference-only cloning when no transcript is available. This intentionally leaves batch and continuation conditioning as future engine-level choices rather than constraining the resource protocol.
